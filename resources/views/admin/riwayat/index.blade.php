                ...
                <tbody class="divide-y divide-beige/10">
                    @foreach ($pinjam as $p)
                        <tr class="hover:bg-beige/5 transition-colors group">
                            ...
                            <td class="p-5">
                                <p class="text-xs font-bold text-DarkChocolate leading-none mb-1">{{ $p->user->name }}</p>
                                <p class="text-[9px] text-MediumBrown/50 font-medium tracking-tight italic">{{ $p->user->email ?? 'Anggota Aktif' }}</p>
                            </td>
                            <td class="p-5 text-center">
                                <p class="text-xs font-bold text-DarkChocolate group-hover:text-Chocolate transition-colors">{{ $p->buku->judul_buku }}</p>
                                <p class="text-[9px] text-MediumBrown/40 font-black uppercase tracking-tighter mt-1">{{ $p->buku->isbn }}</p>
                            </td>
                            <td class="p-5 text-center leading-tight">
                                <div class="inline-block text-left">
                                    <div class="flex items-center gap-2 mb-1">
                                        <i class="fas fa-calendar-alt text-[9px] text-green-500/50"></i>
                                        <span class="text-[10px] font-medium text-DarkChocolate">{{ \Carbon\Carbon::parse($p->tanggal_peminjaman)->format('d M Y') }}</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <i class="fas fa-undo-alt text-[9px] text-red-500/50"></i>
                                        <span class="text-[10px] font-medium text-DarkChocolate">{{ \Carbon\Carbon::parse($p->tanggal_pengembalian)->format('d M Y') }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="p-5 text-center">
                                @php
                                    ...
                                    $class = $statusClasses[$p->status] ?? 'bg-gray-50 text-gray-600 border-gray-100';
                                @endphp
                                <span class="px-3 py-1.5 border {{ $class }} rounded-lg text-[9px] font-black uppercase tracking-widest shadow-sm">
                                    {{ $p->status }}
                                </span>
                            </td>
                            <td class="p-5 pr-8">
