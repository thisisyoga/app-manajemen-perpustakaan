...
@section('content')
    <div class="px-2 space-y-6">
        <div
            class="bg-white p-4 rounded-[28px] border border-beige/40 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
            <form action="{{ url()->current() }}" method="GET" class="flex flex-col md:flex-row gap-3">
                <div class="relative w-full md:w-80 group">
                    <i
                        class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-MediumBrown/30 text-[10px] group-focus-within:text-Chocolate transition-colors"></i>
                    <input type="text" name="user" value="{{ request('user') }}"
                        placeholder="Cari berdasarkan nama, email atau nama lengkap..."
                        ...
                        @foreach ($user as $u)
                            <tr class="hover:bg-beige/5 transition-colors group">
                                ...
                                <td class="p-5">
                                    ...
                                    <p class="text-[9px] text-MediumBrown/40 mt-1 uppercase tracking-tighter">ID:
                                        ANGGOTA-{{ $u->id }}</p>
                                </td>
                                ...
                                <td class="p-5 text-center">
                                    <span
                                        class="px-3 py-1 bg-beige/10 border border-beige/30 text-MediumBrown text-[9px] font-black uppercase tracking-widest rounded-lg group-hover:bg-Chocolate/10 group-hover:text-Chocolate transition-all">
                                        {{ $u->role }}
                                    </span>
                                </td>
                                <td class="p-5 pr-8 text-center">
                                    <form action="{{ route('delete-MDA', $u->id) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button
                                            class="group/btn h-9 px-4 flex items-center gap-2 mx-auto bg-rose-50 text-rose-600 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-rose-600 hover:text-white transition-all active:scale-95 border border-rose-100">
