...
<head>
    ...
    <title>Bukti Pengembalian Resmi - Aksara</title>
    ...
    <style>
        @media print {
            @page {
                size: A4;
                margin: 0; /* Margin nol agar kita bisa kontrol via padding */
            }
            body {
            ...
            /* Kontainer utama dipaksa memenuhi tinggi A4 (~297mm) */
            .print-container {
                width: 210mm;
                height: 297mm;
                padding: 15mm;
            ...
            .print-card {
                flex-grow: 1; /* Memaksa card mengisi ruang yang tersedia */
...
<body class="bg-beige/30 min-h-screen font-sans antialiased text-DarkChocolate">
    ...
    <main class="max-w-4xl mx-auto p-0 md:p-8 print-container">
        <div class="bg-white shadow-2xl overflow-hidden print-card flex flex-col h-full rounded-2xl border border-beige/50">
            ...
            <div class="bg-Chocolate p-10 text-white flex justify-between items-center">
                <div class="flex items-center gap-6">
                    ...
                    <div>
                        <h1 class="text-3xl font-black tracking-tighter uppercase leading-none">AKSARA<span class="text-Caramel">.</span></h1>
                        {{-- <p class="text-[11px] text-beige/70 font-bold tracking-[0.4em] mt-2 uppercase">Official Return Receipt</p> --}}
                    </div>
                </div>
                <div class="text-right border-l-2 border-white/10 pl-8">
                    ...
                    <p class="text-xl font-mono font-bold tracking-tight">
                        #RET-{{ $peminjaman->updated_at->format('Ymd') }}-{{ $peminjaman->id }}
                    </p>
                </div>
            </div>
            ...
            <div class="bg-beige/30 px-10 py-4 border-b border-beige/40 flex justify-between items-center">
                {{-- <div class="flex items-center gap-3">
                    ...
                    <span class="text-xs font-bold text-MediumBrown uppercase tracking-widest">Status: Terverifikasi Sistem</span>
                </div> --}}
                <span class="text-[10px] text-MediumBrown/60 font-medium">Dokumen ini diterbitkan secara elektronik</span>
            </div>
            ...
            <div class="flex-grow flex flex-col">
                <div class="grid grid-cols-4 border-b border-beige/20 bg-white">
                    ...
                    <div class="p-8 bg-red-50/30">
                        <span class="text-MediumBrown/50 text-[9px] font-black uppercase tracking-widest block mb-2">tanggal pengembalian</span>
                        <p
                            class="font-bold text-DarkChocolate text-base italic underline decoration-red-200 bold underline-offset-4">
                            {{ \Carbon\Carbon::parse($peminjaman->updated_at)->format('d M Y') }}
                        </p>
                    </div>
                </div>
                ...
                <div class="p-12 flex-grow">
                    <div class="flex items-start gap-12">
                        <div class="w-40 h-56 bg-slate-50 rounded-lg shadow-xl flex-shrink-0 flex items-center justify-center border border-beige overflow-hidden relative">
                            @if ($peminjaman->buku->cover)
                                <img src="{{ asset('storage/' . $peminjaman->buku->cover) }}" class="w-full h-full object-cover">
                            @else
                                <div class="text-center p-4">
                                    <i class="fas fa-book text-beige text-6xl mb-2"></i>
                                    <p class="text-[8px] text-beige uppercase font-bold">No Cover Available</p>
                                </div>
                            @endif
                        </div>

                        <div class="flex-1 py-2">
                            <span class="bg-emerald-600 text-white px-3 py-1 rounded text-[10px] font-black tracking-widest uppercase italic shadow-sm">Sudah Dikembalikan</span>
                            <h4 class="text-Chocolate font-black text-2xl mt-6 mb-8 leading-[0.9] uppercase tracking-tighter italic">
                                {{ $peminjaman->buku->judul_buku }}
                            </h4>

                            <div class="grid grid-cols-2 gap-x-12 gap-y-8 pt-8 border-t border-beige/40">
                                <div>
                                    <p class="text-MediumBrown/50 text-[10px] font-bold uppercase tracking-widest mb-1">Penulis</p>
                                    <p class="text-DarkChocolate font-bold text-xs uppercase">{{ $peminjaman->buku->penulis }}</p>
                                </div>
                                <div>
                                    <p class="text-MediumBrown/50 text-[10px] font-bold uppercase tracking-widest mb-1">Durasi</p>
                                    <p class="text-DarkChocolate font-bold text-xs uppercase">
                                        {{ floor(\Carbon\Carbon::parse($peminjaman->tanggal_peminjaman)->diffInDays($peminjaman->updated_at)) }} Hari
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            <div class="mt-auto">
                <div class="p-10 bg-slate-50 border-t border-beige/30 flex justify-between items-end">
                    <div class="space-y-4">
                        <p class="text-Chocolate font-black uppercase text-[11px] tracking-widest flex items-center gap-2">
                            <i class="fas fa-certificate text-Caramel"></i> Pernyataan Validasi:
                        </p>
                        <ul class="text-MediumBrown text-xs leading-relaxed space-y-2 font-medium italic opacity-80">
                            <li>• Buku ini telah melalui pengecekan fisik oleh pustakawan.</li>
                            {{-- <li>• Segala denda (jika ada) dinyatakan telah diselesaikan.</li> --}}
                            <li>• Dokumen ini merupakan bukti pengembalian yang sah dan final.</li>
                        </ul>
                    </div>
                ...
                <div class="bg-Chocolate py-4 text-center">
                    <p class="text-beige/60 text-[9px] font-black uppercase tracking-[1em]">Literasi Membangun Negeri &bull; Aksara Library</p>
                </div>
            </div>
        </div>
